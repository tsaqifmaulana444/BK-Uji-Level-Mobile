...
class _BimbinganPribadiState extends State<BimbinganPribadi> {
  ...
  Future<void> deleteDataFromApi(int id) async {
    final response = await http.delete(
        Uri.parse("http://localhost:8000/api/hapus_bimbingan_pribadi/$id"));
    if (response.statusCode == 200) {
      final parsedResponse = json.decode(response.body);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
              "Apakah Anda yakin ingin menghapus bimbingan pribadi ini?"),
          actions: <Widget>[
            TextButton(
                  ...
                  child: TextButton(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                            builder: (context) => FormBimbinganPribadi(user: widget.user)),
                      );
                    },
