...
class _FormBimbinganBelajarState extends State<FormBimbinganBelajar> {
  ...
  Future<void> _fetchGuruData() async {
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          _guruData = List<Map<String, dynamic>>.from(
  ...
  Future<void> _fetchWalasData() async {
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          _walasData = List<Map<String, dynamic>>.from(
              json.decode(response.body)["data"]);
        });
      } else {
        // Handle API error here
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Gagal menambahkan data: ${response.body}"),
            ),
          );
        }
      } catch (e) {
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        decoration:
                            const InputDecoration(labelText: "Wali Kelas"),
